...
cancers=c('ccRCC','BRCA','GBM','CRC','CESC','OV','UCEC','MM','PDAC','HNSCC')
...
piece_id_atac=unique(anno_c$Sample_ATAC[anno_c$Piece_ID==piece_id_rna])
str=cbind(piece_id_rna,piece_id_atac,disease)
all_tab=rbind(all_tab,str)
}
print(disease)
}
all_tab=as.data.frame(all_tab)
colnames(all_tab)[2:3]=c('sample_atac','Disease')
write.table(all_tab, 'snRNA_Sample_IDs.20211219.tsv',sep='\t',quote=F,row.names=F)
